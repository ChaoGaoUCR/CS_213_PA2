...
int main( int argc, char *argv[] )  {
    ...
    case '1':
        ...
        fp_output = fopen("CSROpenMPvec1.txt", "w");
        break;
    case '2':
        ...
        fp_output = fopen("CSROpenMPvec2.txt", "w");
        break;
    case '3':
        ...
        fp_output = fopen("CSROpenMPvec3.txt", "w");
        break;
    }
    ...
    for (int i = 0; i < iteration; i++) // check
    {
        ...
        #pragma omp parallel for num_threads(threads) shared(row_index, partial_sum, vector, val) schedule(guided)
    ...
    printf("CSROpenMP Execution time for %s with %d threads is %f\n",argv[1], threads, end - start);
