...
int main( int argc, char *argv[] )  {
    ...
    case '1':
        ...
        fp_output = fopen("CSCOpenMPvec1.txt", "w");
        break;
    case '2':
        ...
        fp_output = fopen("CSCOpenMPvec2.txt", "w");
        break;
    case '3':
        ...
        fp_output = fopen("CSCOpenMPvec3.txt", "w");
        break;
    }
    ...
    {
        #pragma omp parallel for num_threads(threads) shared(col_index, row_index, partial_sum, val, vector) schedule(guided)
        for (j = 1; j < col+1; j++)
        {
    ...
    printf("CSCParallel Execution time with %d threads for %s is %f\n", threads, argv[1], end - start);
